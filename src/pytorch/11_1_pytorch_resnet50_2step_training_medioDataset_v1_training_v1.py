...
def plot_and_save_training_history(history_tensor, acc_loss_plot_name):
...
#############################################################
#  Loading resnet50 and modifing the last layers

model_ft = models.resnet50(pretrained=True)

# Freezing the model (this should be handled here, otherwise error)
# Reason - we have to freez only the resnet50 layers , not new layers
...
model_ft, history_tensor_pre = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,
                       num_epochs=number_of_epochs_pretraining)

############################################################
#  Starting post training
###########################################################
# to un-freeze the parameters .
for param in model_ft.parameters():
    param.requires_grad = True

model_ft, history_tensor_post = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,
                       num_epochs=number_of_epochs_posttraining)
...
########################################################
#  Saving accuracy and loss plots for two steps
########################################################
pre_processing_plot_name = "pre_process_" + acc_loss_plot_name
plot_and_save_training_history(history_tensor_pre, pre_processing_plot_name)

post_processing_plot_name = "post_processing_" + acc_loss_plot_name
plot_and_save_training_history(history_tensor_post, post_processing_plot_name)
