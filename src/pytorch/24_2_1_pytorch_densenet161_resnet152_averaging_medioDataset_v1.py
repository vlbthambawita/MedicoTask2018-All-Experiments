...
class MyDenseNet(nn.Module):
    ...
    def forward(self, x):
       ...
       # x1 = torch.relu(self.fc1(cat_out))
        x1 = torch.sigmoid(self.fc1(cat_out))
        x1 = torch.sigmoid(self.fc2(x1))
       # x1 = torch.nn.functional.softmax(self.fc2(x1), dim=0)
       # out_1 = torch.cat((x1, x2), 1)
      #  out_1 = torch.relu(self.fc1(out_1))
      #  out = torch.sigmoid(self.fc2(out_1))
        #out = torch.sigmoid(self.fc3(out_1))
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25, model_dir_to_save_last = model_dir):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch == (num_epochs - 1):
            ...
            last_check_point_name = "LAST_" + checkpoint_name_format.format(epoch=epoch, val_acc=epoch_acc)
            print("Last checkpoint name:\n",last_check_point_name)
            model = model.to("cpu")
            torch.save(model.state_dict(), os.path.join(model_dir_to_save_last, last_check_point_name))
...
#################################################################
# Dataparallel should be run without having 2 GPUS also. Otherwise
# when we use two GPUs , it will rise key errors because of
# dataparallel wrapper
##############################
# Another thing is, if are using dataparallel, we have to call it
# before loading the weight file to it
##################################################################

model_ft = nn.DataParallel(model_ft) # This is runing for all the cases
...
# This section is tricky part in this code
# Sometimes it gives loading errors based on the position of
# this code segment(like before or after the multiple GPU settings)
# Found one solution = set parameter strict to False
##########################################################
if arg_mode in ["re-train", "test"]:
    best_weight_file_name = input('Please, enter the best weights value file name:')
    # Strict has to be set to False, other wise it will load exact model keys
    # Then it will not work with defferent number of GPUs
    model_ft.load_state_dict(torch.load(os.path.join(model_dir, best_weight_file_name)), strict=True)
...
############################################################
if arg_mode == "test":
    test_model(model_ft, test_dataloader)  # this exit from the main program
...
if not check_point_name==None:
    ...
    print(type(model_ft))
    model_ft = model_ft.to("cpu") # send model to the CPU before saving it
