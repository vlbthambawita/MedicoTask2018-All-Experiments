...
if arg_mode in ["train", "re-train"]:
    ...
    weights = torch.tensor([[1/819, 1/249]])
...
def test_model(test_model,test_dataloader):
    ...
    weights = torch.tensor([[1 / 3291, 1 / 429]])
    ...
    return y_predicted, testset_predicted_probabilites
...
def prepare_submission_file(image_names, predicted_labels, max_probability, submit_dir, data_classes):
    ...
    submission_dataframe = pd.DataFrame(np.column_stack([image_names, predicted_label_names,max_probability]),
                                   columns=['images', 'labels', 'PROB'])
    #print("image names:{0}".format(image_names))

    submission_dataframe.to_csv(os.path.join(submit_dir, "test1_PROB"), index=False)
...
num_ftrs = model_ft.fc.in_features
model_ft.fc = nn.Linear(num_ftrs, 16) # changed to retrain for 2 classes
...
if arg_mode in ["re-train", "test"]:
    ...
    print("changing the last layer to 2 outputs")
    num_ftrs = model_ft.fc.in_features
    model_ft.fc = nn.Linear(num_ftrs, 2)
    print("Model modification done.. !")
...
############################################################
if arg_mode == "test":
    predicted_labels, predicted_prb = test_model(model_ft, test_dataloader)  # this exit from the main program
    ...
    prepare_submission_file(test_image_names, predicted_labels, predicted_prb ,
                            data_classes= test_datasets.classes, submit_dir=submission_dir )
    exit(0)
...
############################################################
weights = torch.tensor([0.21, 0.79])
