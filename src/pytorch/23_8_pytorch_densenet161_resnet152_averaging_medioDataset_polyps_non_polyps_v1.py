...
class MyNet(nn.Module):
    def __init__(self):
        ...
        self.resnet_model.fc = nn.Linear(self.resnet_num_ftrs, 16) # changed 16 to 2 for 2 class classification
       ...
       # self.densenet_model = pretrained_modle_densnet
        ...
        self.densenet_model.classifier = nn.Linear(self.densenet_num_ftrs, 16) # change 16 to 2 for 2 class classificaiton
...
if arg_mode in ["re-train", "test"]:
    ...
    resnet_num_ftrs = model_ft.resnet_model.fc.in_features
    model_ft.resnet_model.fc = nn.Linear(resnet_num_ftrs, 2)
    densenet_num_ftrs = model_ft.densenet_model.classifier.in_features
    model_ft.densenet_model.classifier = nn.Linear(densenet_num_ftrs, 2)
    print('changes done...!')
...
############################################################
weights = torch.tensor([0.21, 0.79]) #torch.tensor([[1 / 3291, 1 / 429]])
