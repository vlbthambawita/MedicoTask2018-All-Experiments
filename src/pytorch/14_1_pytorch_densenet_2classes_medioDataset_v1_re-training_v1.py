...
def plot_and_save_training_history(history_tensor):
    ...
    pie = df.plot()
    fig = pie.get_figure()
    fig.savefig(os.path.join(plot_dir, "_re-training_" + acc_loss_plot_name))
...
#model_ft, history_tensor, check_point_name = train_model(model_ft, criterion, optimizer_ft,
                     #  num_epochs=number_of_epochs)
